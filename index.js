...
export default class Trimmer extends React.Component {
  ...
  render() {
    ...
    const boundedCurrentPosition = Math.max(currentTime, 0)
    ...
    const actualCurrentOffset = ((boundedCurrentPosition / totalDuration) * trackWidth) + TRACK_PADDING_OFFSET + HANDLE_WIDTHS;
    ...
    return (
      <View style={styles.root}>
        ...
        >
          ...
          left: 0,
          right: 0,
          }}>{this.props.renderCenterLine()}</View>
          {
          ...
          <View style={{
              ...
              overflow: 'visible',
              alignItems: 'center',
              left: Math.max( actualTrimmerOffset, Math.min(actualCurrentOffset + 3, actualTrimmerOffset + actualTrimmerWidth - 3)),
            }}>
              <Text style={{color: 'white', width: 100, textAlign: 'center'}}>{toHHMMSS(currentTime/1000)}</Text>
              <View style={{
                alignSelf: 'center',
                width: 2,
